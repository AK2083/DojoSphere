...
describe('bootstrapNetworkStatus', () => {
  ...
  it('logs unknown heartbeat errors when browser is online', async () => {
    const { bootstrap, api, logging } = await loadBootstrapModule()
    setNavigatorOnline(true)
    vi.mocked(api.heartbeat).mockResolvedValue({
      data: null,
  ...
  it('logs invalid payload responses as unknown', async () => {
    const { bootstrap, api, logging } = await loadBootstrapModule()
    vi.mocked(api.heartbeat).mockResolvedValue({
  ...
  it('tracks Supabase reachability during bootstrap', async () => {
