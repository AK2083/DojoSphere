...
test.describe('ParticipantOverviewSection', { tag: '@regression' }, () => {
  test.beforeEach(async ({ page }) => {
    await setEnglishLanguage(page)
  })

  test('renders empty state without seeded participants', { tag: '@smoke' }, async ({ page }) => {
  ...
  test(
    'renders participant cards when data is available',
    { tag: ['@smoke', '@critical'] },
    async ({ page }) => {
      await page.setViewportSize({ width: 1280, height: 800 })
  ...
  test(
    'navigates to create and edit routes from the section',
    { tag: '@critical' },
    async ({ page }) => {
      await page.setViewportSize({ width: 1280, height: 800 })
      await gotoParticipantsPage(page, { withParticipants: true })
